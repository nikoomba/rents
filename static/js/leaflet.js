...
(function (a) {
    a.L = {
        ...
        function () {
            ...
            for (c = 0, d = a.length; c < d; c++) {
                ...
                if (f) return f[1] === "include" ? "../../dist/" : e.split(b)[0] + "/"
            }
            return ""
        }(),
        noConflict: function () {
    ...
    extend: function (a) {
        return !this._southWest && !this._northEast ? (this._southWest = new L.LatLng(a.lat, a.lng, !0), this._northEast = new L.LatLng(a.lat, a.lng, !0)) : (this._southWest.lat = Math.min(a.lat, this._southWest.lat), this._southWest.lng = Math.min(a.lng, this._southWest.lng), this._northEast.lat = Math.max(a.lat, this._northEast.lat), this._northEast.lng = Math.max(a.lng, this._northEast.lng)), this
    },
    pad: function (a) {
        ...
        return new L.LatLngBounds(new L.LatLng(b.lat - d, b.lng - e), new L.LatLng(c.lat + d, c.lng + e))
    },
    getCenter: function () {
    ...
    equals: function (a) {
        return a ? this._southWest.equals(a.getSouthWest()) && this._northEast.equals(a.getNorthEast()) : !1
    }
    ...
    initialize: function (a, b) {
        ...
        var c = this._container = L.DomUtil.get(a);
        if (c._leaflet) throw Error("Map container is already initialized.");
        c._leaflet = !0, this._initLayout(), L.DomEvent && (this._initEvents(), L.Handler && this._initInteraction(), L.Control && this._initControls()), b = this.options, b.maxBounds && this.setMaxBounds(b.maxBounds);
        var d = b.center,
            e = b.zoom;
        d && typeof e != "undefined" && this.setView(d, e, !0), this._initLayers(b.layers)
    },
    ...
    panBy: function (a) {
        return this.fire("movestart"), this._rawPanBy(a), this.fire("move"), this.fire("moveend")
    },
    ...
    removeLayer: function (a) {
        ...
        if (!this._layers[b]) return;
        return a.onRemove(this), delete this._layers[b], this.options.zoomAnimation && L.TileLayer && a instanceof L.TileLayer && (this._tileLayersNum--, a.off("load", this._onTileLayerLoad, this)), this.attributionControl && a.getAttribution && this.attributionControl.removeAttribution(a.getAttribution()), this.fire("layerremove", {
            layer: a
        })
    },
    hasLayer: function (a) {
    ...
    invalidateSize: function () {
        ...
        var b = a.subtract(this.getSize()).divideBy(2, !0);
        return this._rawPanBy(b), this.fire("move"), clearTimeout(this._sizeTimer), this._sizeTimer = setTimeout(L.Util.bind(c, this), 200), this
    },
    addHandler: function (a, b) {
        if (!b) return;
        return this[a] = new b(this), this.options[a] && this[a].enable(), this
    },
    getCenter: function (a) {
    ...
    getMaxZoom: function () {
        var a = typeof this.options.maxZoom == "undefined" ? Infinity : this.options.maxZoom,
            b = typeof this._layersMaxZoom == "undefined" ? Infinity : this._layersMaxZoom;
        return Math.min(a, b)
    },
    ...
    getPixelBounds: function () {
        ...
        return new L.Bounds(a, a.add(this.getSize()))
    },
    ...
    containerPointToLatLng: function (a) {
        return this.layerPointToLatLng(this.containerPointToLayerPoint(a))
    },
    latLngToContainerPoint: function (a) {
        return this.layerPointToContainerPoint(this.latLngToLayerPoint(a))
    },
    ...
    _initLayout: function () {
        ...
        a.innerHTML = "", a.className += " leaflet-container", L.Browser.touch && (a.className += " leaflet-touch"), this.options.fadeAnimation && (a.className += " leaflet-fade-anim");
    ...
    _resetView: function (a, b, c, d) {
        var e = this._zoom !== b;
        d || (this.fire("movestart"), e && this.fire("zoomstart")), this._zoom = b, this._initialTopLeftPoint = this._getNewTopLeftPoint(a), c ? this._initialTopLeftPoint._add(L.DomUtil.getPosition(this._mapPane)) : L.DomUtil.setPosition(this._mapPane, new L.Point(0, 0)), this._tileLayersToLoad = this._tileLayersNum, this.fire("viewreset", {
            hard: !c
        }), this.fire("move"), (e || d) && this.fire("zoomend"), this.fire("moveend"), this._loaded || (this._loaded = !0, this.fire("load"))
    },
    _initLayers: function (a) {
        a = a instanceof Array ? a : [a], this._layers = {}, this._tileLayersNum = 0;
        var b, c;
        for (b = 0, c = a.length; b < c; b++) this.addLayer(a[b])
    },
    _initControls: function () {
        this.options.zoomControl && (this.zoomControl = new L.Control.Zoom, this.addControl(this.zoomControl)), this.options.attributionControl && (this.attributionControl = new L.Control.Attribution, this.addControl(this.attributionControl))
    },
    _rawPanBy: function (a) {
        var b = L.DomUtil.getPosition(this._mapPane).subtract(a);
        L.DomUtil.setPosition(this._mapPane, b)
    },
    ...
    _fireMouseEvent: function (a) {
        ...
        b === "contextmenu" && L.DomEvent.preventDefault(a);
    ...
    _initInteraction: function () {
        this.addHandler("dragging", L.Map.Drag).addHandler("touchZoom", L.Map.TouchZoom).addHandler("doubleClickZoom", L.Map.DoubleClickZoom).addHandler("scrollWheelZoom", L.Map.ScrollWheelZoom).addHandler("boxZoom", L.Map.BoxZoom)
    },
    ...
    _getNewTopLeftPoint: function (a) {
        var b = this.getSize().divideBy(2);
        return this.project(a)._subtract(b)._round()
    },
    ...
    initialize: function (a, b) {
        L.Util.setOptions(this, b), this._url = a;
        var c = this.options.subdomains;
        typeof c == "string" && (this.options.subdomains = c.split(""))
    },
    onAdd: function (a, b) {
    ...
    onRemove: function (a) {
        a._panes.tilePane.removeChild(this._container), a.off("viewreset", this._resetCallback, this), this.options.updateWhenIdle ? a.off("moveend", this._update, this) : a.off("move", this._limitedUpdate, this), this._container = null, this._map = null
    },
    ...
    setOpacity: function (a) {
        ...
        var b, c = this._tiles;
        if (L.Browser.webkit) for (b in c) c.hasOwnProperty(b) && (c[b].style.webkitTransform += " translate(0,0)")
    },
    ...
    _initContainer: function () {
        var a = this._map._panes.tilePane,
            b = a.firstChild;
        if (!this._container || a.empty) this._container = L.DomUtil.create("div", "leaflet-layer"), this._insertAtTheBottom && b ? a.insertBefore(this._container, b) : a.appendChild(this._container), this.options.opacity < 1 && this._updateOpacity()
    },
    ...
    _addTilesFromCenterOut: function (a) {
        var b = [],
            c = a.getCenter(),
            d, e;
        for (d = a.min.y; d <= a.max.y; d++) for (e = a.min.x; e <= a.max.x; e++) e + ":" + d in this._tiles || b.push(new L.Point(e, d));
        b.sort(function (a, b) {
        ...
        var g, h;
        for (g = 0, h = this._tilesToLoad; g < h; g++) this._addTile(b[g], f);
        this._container.appendChild(f)
    },
    _removeOtherTiles: function (a) {
        var b, c, d, e, f;
        for (e in this._tiles) this._tiles.hasOwnProperty(e) && (b = e.split(":"), c = parseInt(b[0], 10), d = parseInt(b[1], 10), (c < a.min.x || c > a.max.x || d < a.min.y || d > a.max.y) && this._removeTile(e))
    },
    _removeTile: function (a) {
        ...
        this.fire("tileunload", {
            tile: b,
            url: b.src
        }), b.parentNode === this._container && this._container.removeChild(b), this.options.reuseTiles && this._unusedTiles.push(b), b.src = L.Util.emptyImageUrl, delete this._tiles[a]
    },
    _addTile: function (a, b) {
        var c = this._map.getZoom(),
            d = a.x + ":" + a.y,
            e = Math.pow(2, this._getOffsetZoom(c));
        if (!this.options.continuousWorld) if (!this.options.noWrap) a.x = (a.x % e + e) % e;
        else if (a.x < 0 || a.x >= e || a.y < 0 || a.y >= e) {
            this._tilesToLoad--;
            return
        }
        var f = this._getTile();
        L.DomUtil.setPosition(f, this._getTilePos(a)), this._tiles[d] = f, this.options.scheme === "tms" && (a.y = e - a.y - 1), this._loadTile(f, a, c), b.appendChild(f)
    },
    _getOffsetZoom: function (a) {
        var b = this.options;
        return a = b.zoomReverse ? b.maxZoom - a : a, a + b.zoomOffset
    },
    ...
    _createTileProto: function () {
        var a = this._tileImg = L.DomUtil.create("img", "leaflet-tile");
        a.galleryimg = "no";
        var b = this.options.tileSize;
        a.style.width = b + "px", a.style.height = b + "px"
    },
    _getTile: function () {
    ...
    initialize: function (a, b) {
        this._url = a;
        var c = L.Util.extend({}, this.defaultWmsParams);
        c.width = c.height = this.options.tileSize;
        for (var d in b) this.options.hasOwnProperty(d) || (c[d] = b[d]);
        this.wmsParams = c, L.Util.setOptions(this, b)
    },
    onAdd: function (a, b) {
        var c = parseFloat(this.wmsParams.version) < 1.3 ? "srs" : "crs";
        this.wmsParams[c] = a.options.crs.code, L.TileLayer.prototype.onAdd.call(this, a, b)
    },
    getTileUrl: function (a, b) {
        var c = this._map,
            d = c.options.crs,
            ...
            f = a.multiplyBy(e),
            g = f.add(new L.Point(e, e)),
            h = c.unproject(f, b, !0),
            i = c.unproject(g, b, !0),
            j = d.project(h),
            k = d.project(i),
            l = [j.x, k.y, k.x, j.y].join(",");
        return this._url + L.Util.getParamString(this.wmsParams) + "&bbox=" + l
    }
    ...
    redraw: function () {
        var a, b = this._tiles;
        for (a in b) b.hasOwnProperty(a) && this._redrawTile(b[a])
    },
    _redrawTile: function (a) {
        this.drawTile(a, a._tilePoint, a._zoom)
    },
    _createTileProto: function () {
        var a = this._canvasProto = L.DomUtil.create("canvas", "leaflet-tile"),
    ...
    onAdd: function (a) {
        this._map = a, this._image || this._initImage(), a._panes.overlayPane.appendChild(this._image), a.on("viewreset", this._reset, this), this._reset()
    },
    onRemove: function (a) {
    ...
    _reset: function () {
        var a = this._image,
            ...
            c = this._map.latLngToLayerPoint(this._bounds.getSouthEast()).subtract(b);
        L.DomUtil.setPosition(a, b), a.style.width = c.x + "px", a.style.height = c.y + "px"
    },
    ...
    _createIcon: function (a) {
        var b = this._createImg(this.options[a + "Url"]);
        return this._setIconStyles(b, a), b
    },
    _setIconStyles: function (a, b) {
        var c = this.options,
            d = c[b + "Size"],
            e = c.iconAnchor || d.divideBy(2, !0);
        b === "shadow" && e._add(c.shadowOffset), a.className = "leaflet-marker-" + b + " " + c.className, a.style.marginLeft = -e.x + "px", a.style.marginTop = -e.y + "px", c.iconSize && (a.style.width = d.x + "px", a.style.height = d.y + "px")
    },
    ...
    onAdd: function (a) {
        this._map = a, a.on("viewreset", this._reset, this), this._initIcon(), this._reset()
    },
    onRemove: function (a) {
        this._removeIcon(), this.closePopup && this.closePopup(), a.off("viewreset", this._reset, this), this._map = null
    },
    getLatLng: function () {
        return this._latlng
    },
    setLatLng: function (a) {
        this._latlng = a, this._reset(), this._popup && this._popup.setLatLng(a)
    },
    setZIndexOffset: function (a) {
        this.options.zIndexOffset = a, this._reset()
    },
    setIcon: function (a) {
    ...
    _initIcon: function () {
        var a = this.options;
        this._icon || (this._icon = a.icon.createIcon(), a.title && (this._icon.title = a.title), this._initInteraction(), this._updateOpacity()), this._shadow || (this._shadow = a.icon.createShadow());
        var b = this._map._panes;
        b.markerPane.appendChild(this._icon), this._shadow && b.shadowPane.appendChild(this._shadow)
    },
    _removeIcon: function () {
        var a = this._map._panes;
        a.markerPane.removeChild(this._icon), this._shadow && a.shadowPane.removeChild(this._shadow), this._icon = this._shadow = null
    },
    _reset: function () {
        var a = this._icon;
        if (!a) return;
        var b = this._map.latLngToLayerPoint(this._latlng).round();
        L.DomUtil.setPosition(a, b), this._shadow && L.DomUtil.setPosition(this._shadow, b), a.style.zIndex = b.y + this.options.zIndexOffset
    },
    _initInteraction: function () {
        if (!this.options.clickable) return;
        var a = this._icon,
            b = ["dblclick", "mousedown", "mouseover", "mouseout"];
        a.className += " leaflet-clickable", L.DomEvent.addListener(a, "click", this._onMouseClick, this);
        for (var c = 0; c < b.length; c++) L.DomEvent.addListener(a, b[c], this._fireMouseEvent, this);
    ...
    onAdd: function (a) {
        this._map = a, this._container || this._initLayout(), this._updateContent(), this._container.style.opacity = "0", a._panes.popupPane.appendChild(this._container), a.on("viewreset", this._updatePosition, this), a.options.closePopupOnClick && a.on("preclick", this._close, this), this._update(), this._container.style.opacity = "1"
    },
    onRemove: function (a) {
        a._panes.popupPane.removeChild(this._container), L.Util.falseFn(this._container.offsetWidth), a.off("viewreset", this._updatePosition, this).off("click", this._close, this), this._container.style.opacity = "0", this._map = null
    },
    setLatLng: function (a) {
        return this._latlng = a, this._update(), this
    },
    setContent: function (a) {
        return this._content = a, this._update(), this
    },
    _close: function () {
        this._map && this._map.closePopup()
    },
    _initLayout: function () {
        var a = "leaflet-popup",
            b = this._container = L.DomUtil.create("div", a + " " + this.options.className),
            c;
        this.options.closeButton && (c = this._closeButton = L.DomUtil.create("a", a + "-close-button", b), c.href = "#close", L.DomEvent.addListener(c, "click", this._onCloseButtonClick, this));
        var d = this._wrapper = L.DomUtil.create("div", a + "-content-wrapper", b);
        L.DomEvent.disableClickPropagation(d), this._contentNode = L.DomUtil.create("div", a + "-content", d), L.DomEvent.addListener(this._contentNode, "mousewheel", L.DomEvent.stopPropagation), this._tipContainer = L.DomUtil.create("div", a + "-tip-container", b), this._tip = L.DomUtil.create("div", a + "-tip", this._tipContainer)
    },
    _update: function () {
        if (!this._map) return;
    ...
    _updateLayout: function () {
        ...
        a.style.width = "", a.style.whiteSpace = "nowrap";
        var b = a.offsetWidth;
        b = Math.min(b, this.options.maxWidth), b = Math.max(b, this.options.minWidth), a.style.width = b + 1 + "px", a.style.whiteSpace = "", a.style.height = "";
        var c = a.offsetHeight,
            d = this.options.maxHeight,
            e = " leaflet-popup-scrolled";
        d && c > d ? (a.style.height = d + "px", a.className += e) : a.className = a.className.replace(e, ""), this._containerWidth = this._container.offsetWidth
    },
    ...
    _adjustPan: function () {
        ...
        var a = this._map,
            ...
            d = new L.Point(this._containerLeft, -b - this._containerBottom),
            e = a.layerPointToContainerPoint(d),
            f = new L.Point(0, 0),
            g = this.options.autoPanPadding,
            h = a.getSize();
        e.x < 0 && (f.x = e.x - g.x), e.x + c > h.x && (f.x = e.x + c - h.x + g.x), e.y < 0 && (f.y = e.y - g.y), e.y + b > h.y && (f.y = e.y + b - h.y + g.y), (f.x || f.y) && a.panBy(f)
    },
    ...
    bindPopup: function (a, b) {
        ...
        }, b), this._popup || this.on("click", this.openPopup, this), this._popup = (new L.Popup(b, this)).setContent(a), this
    },
    ...
    openPopup: function (a) {
        return this.closePopup(), this._popup = a, this.addLayer(a).fire("popupopen", {
            popup: this._popup
        })
    },
    closePopup: function () {
        return this._popup && (this.removeLayer(this._popup).fire("popupclose", {
            popup: this._popup
    ...
    onRemove: function (a) {
        this._iterateLayers(a.removeLayer, a), this._map = null
    },
    ...
    _initEvents: function (a) {
        var b = ["click", "dblclick", "mouseover", "mouseout"],
            c, d;
        for (c = 0, d = b.length; c < d; c++) a.on(b[c], this._propagateEvent, this)
    },
    ...
    onAdd: function (a) {
        this._map = a, this._initElements(), this._initEvents(), this.projectLatlngs(), this._updatePath(), a.on("viewreset", this.projectLatlngs, this).on("moveend", this._updatePath, this)
    },
    onRemove: function (a) {
        this._map = null, a._pathRoot.removeChild(this._container), a.off("viewreset", this.projectLatlngs, this).off("moveend", this._updatePath, this)
    },
    ...
    _updatePathViewport: function () {
        var a = L.Path.CLIP_PADDING,
            ...
            d = c.multiplyBy(-1)._subtract(b.multiplyBy(a)),
    ...
    _initPath: function () {
        this._container = this._createElement("g"), this._path = this._createElement("path"), this._container.appendChild(this._path), this._map._pathRoot.appendChild(this._container)
    },
    _initStyle: function () {
    ...
    _updateStyle: function () {
        this.options.stroke ? (this._path.setAttribute("stroke", this.options.color), this._path.setAttribute("stroke-opacity", this.options.opacity), this._path.setAttribute("stroke-width", this.options.weight)) : this._path.setAttribute("stroke", "none"), this.options.fill ? (this._path.setAttribute("fill", this.options.fillColor || this.options.color), this._path.setAttribute("fill-opacity", this.options.fillOpacity)) : this._path.setAttribute("fill", "none")
    },
    ...
    _onMouseClick: function (a) {
        ...
        a.type === "contextmenu" && L.DomEvent.preventDefault(a), this._fireMouseEvent(a)
    },
    _fireMouseEvent: function (a) {
    ...
    _initPathRoot: function () {
        this._pathRoot || (this._pathRoot = L.Path.prototype._createElement("svg"), this._panes.overlayPane.appendChild(this._pathRoot), this.on("moveend", this._updateSvgViewport), this._updateSvgViewport())
    },
    ...
    var b = a.firstChild;
    return b.style.behavior = "url(#default#VML)", b && typeof b.adj == "object"
    ...
    _initPath: function () {
        ...
        a.className += " leaflet-vml-shape" + (this.options.clickable ? " leaflet-clickable" : ""), a.coordsize = "1 1", this._path = this._createElement("path"), a.appendChild(this._path), this._map._pathRoot.appendChild(a)
    },
    _initStyle: function () {
        var a = this._container,
            b, c;
        this.options.stroke && (b = this._stroke = this._createElement("stroke"), b.endcap = "round", a.appendChild(b)), this.options.fill && (c = this._fill = this._createElement("fill"), a.appendChild(c)), this._updateStyle()
    },
    _updateStyle: function () {
        var a = this._stroke,
            b = this._fill,
            c = this.options,
            d = this._container;
        d.stroked = c.stroke, d.filled = c.fill, c.stroke && (a.weight = c.weight + "px", a.color = c.color, a.opacity = c.opacity), c.fill && (b.color = c.fillColor || c.color, b.opacity = c.fillOpacity)
    },
    _updatePath: function () {
    ...
    _initPathRoot: function () {
        ...
        var a = this._pathRoot = document.createElement("div");
    ...
    _updatePath: function () {
        ...
        var a = this._ctx,
            b = this.options;
        this._drawPath(), a.save(), this._updateStyle(), b.fill && (b.fillOpacity < 1 && (a.globalAlpha = b.fillOpacity), a.fill()), b.stroke && (b.opacity < 1 && (a.globalAlpha = b.opacity), a.stroke()), a.restore()
    },
    _initEvents: function () {
    ...
    onRemove: function (a) {
        a.off("viewreset", this._projectLatlngs, this).off("moveend", this._updatePath, this).fire("moveend")
    }
    ...
    setLatLngs: function (a) {
        return this._latlngs = a, this.redraw()
    },
    addLatLng: function (a) {
        return this._latlngs.push(a), this.redraw()
    },
    ...
    _initEvents: function () {
        L.Polyline.superclass._initEvents.call(this), L.Handler.PolyEdit && (this.editing = new L.Handler.PolyEdit(this))
    },
    ...
    setLatLng: function (a) {
        return this._latlng = a, this.redraw()
    },
    ...
    projectLatlngs: function () {
        var a = this._getLngRadius(),
            b = new L.LatLng(this._latlng.lat, this._latlng.lng - a, !0),
            c = this._map.latLngToLayerPoint(b);
        this._point = this._map.latLngToLayerPoint(this._latlng), this._radius = Math.round(this._point.x - c.x)
    },
    getBounds: function () {
        var a = this._map,
            b = this._radius * Math.cos(Math.PI / 4),
            c = a.project(this._latlng),
            d = new L.Point(c.x - b, c.y + b),
            e = new L.Point(c.x + b, c.y - b),
            f = a.getZoom(),
            g = a.unproject(d, f, !0),
            h = a.unproject(e, f, !0);
        return new L.LatLngBounds(g, h)
    },
    getPathString: function () {
        var a = this._point,
            b = this._radius;
    ...
    setRadius: function (a) {
        return this._radius = a, this.redraw()
    }
    ...
    _drawPath: function () {
        var a = this._point;
        this._ctx.beginPath(), this._ctx.arc(a.x, a.y, this._radius, 0, Math.PI * 2, !1)
    },
    _containsPoint: function (a) {
        var b = this._point,
    ...
    addGeoJSON: function (a) {
        var b = a.features,
            c, d;
        if (b) {
            for (c = 0, d = b.length; c < d; c++) this.addGeoJSON(b[c]);
            return
        }
        var e = a.type === "Feature",
            f = e ? a.geometry : a,
            g = L.GeoJSON.geometryToLayer(f, this.options.pointToLayer);
    ...
    geometryToLayer: function (a, b) {
        ...
        switch (a.type) {
        case "Point":
            return e = this.coordsToLatLng(c), b ? b(e) : new L.Marker(e);
        case "MultiPoint":
            for (g = 0, h = c.length; g < h; g++) e = this.coordsToLatLng(c[g]), i = b ? b(e) : new L.Marker(e), d.push(i);
            return new L.FeatureGroup(d);
        case "LineString":
            return f = this.coordsToLatLngs(c), new L.Polyline(f);
        case "Polygon":
            return f = this.coordsToLatLngs(c, 1), new L.Polygon(f);
        case "MultiLineString":
            return f = this.coordsToLatLngs(c, 1), new L.MultiPolyline(f);
        case "MultiPolygon":
            return f = this.coordsToLatLngs(c, 2), new L.MultiPolygon(f);
        case "GeometryCollection":
            for (g = 0, h = a.geometries.length; g < h; g++) i = this.geometryToLayer(a.geometries[g], b), d.push(i);
            return new L.FeatureGroup(d);
        default:
    ...
    removeListener: function (a, b, c) {
        var d = L.Util.stamp(c),
            ...
            f = a[e];
        if (!f) return;
        return L.Browser.touch && b === "dblclick" && this.removeDoubleTapListener ? this.removeDoubleTapListener(a, d) : "removeEventListener" in a ? b === "mousewheel" ? (a.removeEventListener("DOMMouseScroll", f, !1), a.removeEventListener(b, f, !1)) : b === "mouseenter" || b === "mouseleave" ? a.removeEventListener(b === "mouseenter" ? "mouseover" : "mouseout", f, !1) : a.removeEventListener(b, f, !1) : "detachEvent" in a && a.detachEvent("on" + b, f), a[e] = null, this
    },
    ...
    disableClickPropagation: function (a) {
        return L.DomEvent.addListener(a, L.Draggable.START, L.DomEvent.stopPropagation).addListener(a, "click", L.DomEvent.stopPropagation).addListener(a, "dblclick", L.DomEvent.stopPropagation)
    },
    preventDefault: function (a) {
        return a.preventDefault ? a.preventDefault() : a.returnValue = !1, this
    },
    stop: function (a) {
        return L.DomEvent.preventDefault(a).stopPropagation(a)
    },
    ...
    disable: function () {
        if (!this._enabled) return;
        L.DomEvent.removeListener(this._dragStartTarget, L.Draggable.START, this._onDown), this._enabled = !1, this._moved = !1
    },
    _onDown: function (a) {
    ...
    _setMovingCursor: function () {
        this._bodyCursor = document.body.style.cursor, this._dragStartTarget.style.cursor = document.body.style.cursor = "move"
    },
    _restoreCursor: function () {
        document.body.style.cursor = this._bodyCursor, this._dragStartTarget.style.cursor = ""
    },
    ...
    _onTouchStart: function (a) {
        var b = this._map;
        if (!a.touches || a.touches.length !== 2 || b._animatingZoom) return;
        var c = b.mouseEventToContainerPoint(a.touches[0]),
            d = b.mouseEventToContainerPoint(a.touches[1]),
            e = b.containerPointToLayerPoint(b.getSize().divideBy(2));
        this._startCenter = c.add(d).divideBy(2, !0), this._startDist = c.distanceTo(d), this._moved = !1, this._zooming = !0, this._centerOffset = e.subtract(this._startCenter), L.DomEvent.addListener(document, "touchmove", this._onTouchMove, this).addListener(document, "touchend", this._onTouchEnd, this), L.DomEvent.preventDefault(a)
    },
    _onTouchMove: function (a) {
        if (!a.touches || a.touches.length !== 2) return;
        var b = this._map;
        this._moved || (b._mapPane.className += " leaflet-zoom-anim", b.fire("zoomstart").fire("movestart")._prepareTileBg(), this._moved = !0);
        var c = b.mouseEventToContainerPoint(a.touches[0]),
            d = b.mouseEventToContainerPoint(a.touches[1]);
        this._scale = c.distanceTo(d) / this._startDist, this._delta = c.add(d).divideBy(2, !0).subtract(this._startCenter), b._tileBg.style.webkitTransform = L.DomUtil.getTranslateString(this._delta) + " " + L.DomUtil.getScaleString(this._scale, this._startCenter), L.DomEvent.preventDefault(a)
    },
    _onTouchEnd: function (a) {
        if (!this._moved || !this._zooming) return;
        this._zooming = !1, L.DomEvent.removeListener(document, "touchmove", this._onTouchMove).removeListener(document, "touchend", this._onTouchEnd);
        var b = this._centerOffset.subtract(this._delta).divideBy(this._scale),
            c = this._map.getPixelOrigin().add(this._startCenter).add(b),
            d = this._map.unproject(c),
            e = this._map.getZoom(),
            f = Math.log(this._scale) / Math.LN2,
            g = f > 0 ? Math.ceil(f) : Math.floor(f),
            h = this._map._limitZoom(e + g),
            i = Math.pow(2, h - e);
        this._map._runAnimation(d, h, i / this._scale, this._startCenter.add(b))
    }
    ...
    _onMouseMove: function (a) {
        var b = this._startLayerPoint,
            ...
            e = d.subtract(b),
            f = new L.Point(Math.min(d.x, b.x), Math.min(d.y, b.y));
        L.DomUtil.setPosition(c, f), c.style.width = Math.abs(e.x) - 4 + "px", c.style.height = Math.abs(e.y) - 4 + "px"
    },
    _onMouseUp: function (a) {
        this._pane.removeChild(this._box), this._container.style.cursor = "", L.DomUtil.enableTextSelection(), L.DomEvent.removeListener(document, "mousemove", this._onMouseMove).removeListener(document, "mouseup", this._onMouseUp);
        var b = this._map,
            c = b.mouseEventToLayerPoint(a),
            d = new L.LatLngBounds(b.layerPointToLatLng(this._startLayerPoint), b.layerPointToLatLng(c));
        b.fitBounds(d)
    }
    ...
    initialize: function (a, b) {
        this._poly = a, L.Util.setOptions(this, b)
    },
    addHooks: function () {
    ...
    _initMarkers: function () {
        this._markerGroup = new L.LayerGroup, this._markers = [];
        ...
        var f, g;
        for (b = 0, c = d - 1; b < d; c = b++) {
            if (b === 0 && !(L.Polygon && this._poly instanceof L.Polygon)) continue;
            f = this._markers[c], g = this._markers[b], this._createMiddleMarker(f, g), this._updatePrevNext(f, g)
        }
    },
    _createMarker: function (a, b) {
        ...
        return c._origLatLng = a, c._index = b, c.on("drag", this._onMarkerDrag, this), this._markerGroup.addLayer(c), c
    },
    _onMarkerDrag: function (a) {
        var b = a.target;
        L.Util.extend(b._origLatLng, b._latlng), b._middleLeft && b._middleLeft.setLatLng(this._getMiddleLatLng(b._prev, b)), b._middleRight && b._middleRight.setLatLng(this._getMiddleLatLng(b, b._next)), this._poly.redraw()
    },
    _onMarkerClick: function (a) {
        var b = a.target,
            c = b._index;
        this._createMiddleMarker(b._prev, b._next), this._updatePrevNext(b._prev, b._next), this._markerGroup.removeLayer(b._middleLeft).removeLayer(b._middleRight).removeLayer(b), this._poly.spliceLatLngs(c, 1), this._updateIndexes(c, -1), this._poly.fire("edit")
    },
    _updateIndexes: function (a, b) {
        this._markerGroup._iterateLayers(function (c) {
    ...
    _createMiddleMarker: function (a, b) {
        function e() {
            var e = b._index;
            d._index = e, d.off("click", g).on("click", this._onMarkerClick, this), this._poly.spliceLatLngs(e, 0, c), this._markers.splice(e, 0, d), this._poly.fire("edit"), d.setOpacity(1), this._updateIndexes(e, 1), b._index++, this._updatePrevNext(a, d), this._updatePrevNext(d, b)
        }
        function f() {
            d.off("dragstart", e, this), d.off("dragend", f, this), this._createMiddleMarker(a, d), this._createMiddleMarker(d, b)
        }
        function g() {
        ...
        d.setOpacity(.6), a._middleRight = b._middleLeft = d, d.on("click", g, this).on("dragstart", e, this).on("dragend", f, this), this._markerGroup.addLayer(d)
    },
    ...
    _getMiddleLatLng: function (a, b) {
        var c = this._poly._map,
            d = c.latLngToLayerPoint(a.getLatLng()),
            e = c.latLngToLayerPoint(b.getLatLng());
        return c.layerPointToLatLng(d._add(e).divideBy(2))
    }
    ...
    setPosition: function (a) {
        this.options.position = a, this._map && (this._map.removeControl(this), this._map.addControl(this))
    }
}), L.Map.include({
    addControl: function (a) {
        var b = a.onAdd(this);
        a._container = b, a._map = this;
        var c = a.getPosition(),
            d = this._controlCorners[c];
        return L.DomUtil.addClass(b, "leaflet-control"), c.indexOf("bottom") !== -1 ? d.insertBefore(b, d.firstChild) : d.appendChild(b), this
    },
    removeControl: function (a) {
        var b = a.getPosition(),
            c = this._controlCorners[b];
        return c.removeChild(a._container), a._map = null, a.onRemove && a.onRemove(this), this
    },
    _initControlPos: function () {
        var a = "leaflet-top",
            ...
            c = "leaflet-left",
            d = "leaflet-right",
            e = L.DomUtil.create("div", "leaflet-control-container", this._container),
            f = this._controlCorners = {};
        f.topleft = L.DomUtil.create("div", a + " " + c, e), f.topright = L.DomUtil.create("div", a + " " + d, e), f.bottomleft = L.DomUtil.create("div", b + " " + c, e), f.bottomright = L.DomUtil.create("div", b + " " + d, e)
    }
    ...
    onAdd: function (a) {
        var b = "leaflet-control-zoom",
            ...
            d = this._createButton("Zoom in", b + "-in", a.zoomIn, a),
            e = this._createButton("Zoom out", b + "-out", a.zoomOut, a);
        return c.appendChild(d), c.appendChild(e), c
    },
    ...
    initialize: function (a, b) {
        L.Util.setOptions(this, b), this._prefix = a || 'Powered by <a href="http://leaflet.cloudmade.com">Leaflet</a>', this._attributions = {}
    },
    onAdd: function (a) {
        return this._container = L.DomUtil.create("div", "leaflet-control-attribution"), L.DomEvent.disableClickPropagation(this._container), this._map = a, this._update(), this._container
    },
    ...
    onAdd: function (a) {
        return this._map = a, this._initLayout(), this._update(), this._container
    },
    ...
    _zoomToIfCenterInView: function (a, b, c) {
        ...
        this._mapPane.className += " leaflet-zoom-anim", this.fire("movestart").fire("zoomstart"), this._prepareTileBg();
        var f = this.containerPointToLayerPoint(this.getSize().divideBy(2)),
            g = f.add(e);
        return this._runAnimation(a, b, d, g), !0
    },
    _runAnimation: function (a, b, c, d) {
        ...
        if (L.Browser.gecko || window.opera) f.style[e] += " translate(0,0)";
        var g;
        L.Browser.android ? (f.style[e + "Origin"] = d.x + "px " + d.y + "px", g = "scale(" + c + ")") : g = L.DomUtil.getScaleString(c, d), L.Util.falseFn(f.offsetWidth);
        var h = {};
        h[e] = f.style[e] + " " + g, f.transition.run(h)
    },
    _prepareTileBg: function () {
        var a = this._tilePane,
            b = this._tileBg;
        b || (b = this._tileBg = this._createPane("leaflet-tile-pane", this._mapPane), b.style.zIndex = 1), b.style[L.DomUtil.TRANSFORM] = "", b.style.visibility = "hidden", b.empty = !0, a.empty = !1, this._tilePane = this._panes.tilePane = b;
        var c = this._tileBg = a;
        c.transition || (c.transition = new L.Transition(this._tileBg, {
            duration: .25,
            easing: "cubic-bezier(0.25,0.1,0.25,0.75)"
        }), c.transition.on("end", this._onZoomTransitionEnd, this)), this._stopLoadingBgTiles()
    },
    _stopLoadingBgTiles: function () {
        var a = Array.prototype.slice.call(this._tileBg.getElementsByTagName("img")),
            b, c, d;
        for (b = 0, c = a.length; b < c; b++) d = a[b], d.complete || (d.onload = L.Util.falseFn, d.onerror = L.Util.falseFn, d.src = L.Util.emptyImageUrl, d.parentNode.removeChild(d))
    },
    ...
    locate: function (a) {
        a = this._locationOptions = L.Util.extend(this._defaultLocateOptions, a);
    ...
    _handleGeolocationResponse: function (a) {
        var b = 180 * a.coords.accuracy / 4e7,
            ...
            j = this._locationOptions;
        if (j.setView) {
            var k = Math.min(this.getBoundsZoom(i), j.maxZoom);
            this.setView(f, k)
        }
